...
class glusteractions():
    ...
    def gluster_install(self, version):
        ...
        for node in nodes:
            ...
            if status1:
                ...
                status2 = run_helper.run_command(
                    node, gluster_install_command, True)
                time.sleep(20)
                if status2:
        ...
        if status1 or status2:
            logger.critical(
                ...
                "binary installation failed on :%s ",
                failed_package_nodes, failed_install_nodes)
        else:
            logger.info(
    ...
    def create_gluster_volume(self, start=True):
        ...
        if number_bricks > number_nodes:
            ...
            logger.critical(
                "The support to have more than 1 brick per "
        ...
        for i in range(0, number_bricks):
            brick = "%s:%s/%s_brick%d" % (
                nodes[node_index], export_dir, volname, i)
        ...
        vol_create_cmd = (
            "gluster --mode=script volume create %s %s %s "
            "transport %s %s force" %
            (volname, replica_count, stripe_count,
                trans_type, ' '.join(brick_list)))

        flag = 0
        for node in nodes:
            status = run_helper.run_command(
                node, 'pgrep glusterd || glusterd', True)
            if status:
                logger.error(
                    'glusterd can not be started in node: %s', node)
                flag = 1
        ...
        for node in nodes:
            if node != masternode:
                status = run_helper.run_command(
                    masternode, 'gluster peer probe ' + node, False)
                time.sleep(20)
                if status:
        ...
        if status == 0 and start:
            status = run_helper.run_command(
                masternode,
                "gluster --mode=script volume start %s" % volname,
                False)
