...
def usage():
    logger.debug ('Usage: run_helper.py {[-c "file to be copied:path in destination machine"] [-r "command to be run"]}')
    return 0
...
def get_nodes_ip():
    ...
    try:
        ...
        logger.critical ( 'unable to retrive the server ip address from configfile. Please set SERVERS_IP_ADDRS in configfile')
        sys.exit(1)
...
def get_mountpoint():
    try:
        ...
        logger.critical ( 'unable to find the mount point. Please set the MOUNTPOINT in configfile')
        sys.exit(1)
    ...
    if mountpoint in invalid_mountpoints:
        logger.critical ("%s  + ' is not a valid mountpoint. Please provide a valid mountpoint. Aborting...",mountpoint)
        sys.exit(1)
...
#NOTE: I'm not sure how much of above code is robust. Because if the remote machine sends back enough
# data to fill the buffer of 'channel file object' then, host (this machine) may hang forever.
# Need a better way to handle this issue. Current code just assumes that the remote machine doesn't send lot of data.
...
def main():
    opt = arg = []
    try:
        opt, arg = getopt.getopt(sys.argv[1:], "c:r:C:R:p:", ["copy=", "run=", "Run=", "Copy=", "parallel="])
